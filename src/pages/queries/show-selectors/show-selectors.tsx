import { IResourceComponentsProps, useOne, useMany, useUpdate, useNavigation, useRefineContext, useBreadcrumb } from '@pankod/refine-core';
import { Show,   Divider,
  Typography,
  UrlField,
  Space,
  ...
  SaveButton,
  useModalForm,
  CreateButton, } from '@pankod/refine-antd';
...
export const ShowSelectors: React.FC<IResourceComponentsProps> = () => {
  ...
  */
  ...
  const parentResource = `queries/${record?.id || 0}`;
  const resource = `${parentResource}/selectors`;

    const { data: selectorsData, isLoading: selectorsIsLoading } = useMany<ISelector>({
      resource,
      ids: [],
      queryOptions: {
        enabled: !!record ,
...
<Space
    ...
    <Title level={4}>Start Url</Title>
      ...
      <SelectorsTree resource={resource} selectors={selectorsData?.data || []} />
      </Show>
      </>
  );
};
